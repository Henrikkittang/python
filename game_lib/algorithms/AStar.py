...
def findNeighbours(node: Node, grid: list, diagonal: bool):
            ...
            if t != 0 or q != 0:
                if cur_pos[1] + t >= 0 and cur_pos[1] + t < len(grid) and cur_pos[0] + q >= 0 and cur_pos[0] + q < len(grid[0]):
                    if grid[cur_pos[1] + t][cur_pos[0] + q] == 0:
                        yield (cur_pos[0] + q, cur_pos[1] + t)

 

def AStar(grid: list, start_pos: tuple, end_pos: tuple, diagonal: bool=True):
    ''' 
        Finds the shortes path between start_pos and end_pos in a 2d grid  
        represented by as a nested list using the A* algorithm. 
        Can by default search diagonaly
    '''
    ...
    while cur_node.position != end_pos:
        ...
        childrenPos = findNeighbours(cur_node, grid, diagonal)
